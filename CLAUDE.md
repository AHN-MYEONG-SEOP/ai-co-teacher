...
```
### 코드 수정 전 → MODULE_MAP.md 반드시 참조
```
- 수정할 기능이 어느 파일의 몇 번 줄에 있는지 MODULE_MAP.md에서 먼저 확인
- 함수명으로 검색해서 정확한 위치 파악 후 수정
- 관련 함수/파일도 함께 확인 (의존성)
```

### 매 작업 완료 시 → MODULE_MAP.md 자동 업데이트
```
- 새 파일/함수 생성 시 → MODULE_MAP.md 해당 섹션에 추가
- 함수 이름/위치(줄 번호) 변경 시 → 즉시 업데이트
- 파일 삭제/이동 시 → MODULE_MAP.md에서 제거
- 기능별 수정 가이드에 새 패턴 추가
- 업데이트 이력 표에 날짜/버전/변경내용 추가


### 설계 문서 관리 규칙 (docs/ 폴더)
```
- 설계와 다르게 구현할 때 → docs/ 해당 문서에 변경 표시
  예: v2026-06-06.XX에 변경됨 / 변경 전: ... / 변경 후: ...
- 세션 종료 전 → 설계 문서와 달라진 부분 체크
- 사소한 변경은 무시, 구조/방향이 바뀐 것만 업데이트
- docs/ 문서 상단 마지막 동기화 버전 업데이트
```
